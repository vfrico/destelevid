...
class RTVCYL(object):
    ...
    def procesarDescarga(self):
        ...
        if streamHTML.find("&videoId=") != -1:
            ...
            name += ext
        elif streamHTML.find("NicePlayer.js?") != -1 or streamHTML.find("nicePlayer.js?") != -1:
            new = True
            try:
                urlJS = streamHTML.split("NicePlayer.js?")[0].split("\"")[-1] + \
                "NicePlayer.js?" + streamHTML.split("NicePlayer.js?")[1].split("\"")[0]
            except IndexError:
                urlJS = streamHTML.split("nicePlayer.js?")[0].split("\"")[-1] + \
                "nicePlayer.js?" + streamHTML.split("nicePlayer.js?")[1].split("\"")[0]
            streamJS = self.__descHTML(urlJS)
            url = self.URL_RTMP + streamJS.split("var fileFlash = \"")[1].split("\"")[0]
            name = streamHTML.split("<title>")[1].split("<")[0].strip()
            name += "." + url.split(".")[-1]
        else:
        ...
        if new:
            return [url, name, "-p", self._URL_recibida]
        else:
            return [url, name]
