...
class CSur(object):
    ...
    def __alacarta(self):
        ...
        if xmlStream[xmlStream.find("<url>")+1:].find("<url>") != -1:
            printt(u"[INFO] Varios Vídeos")
            a = xmlStream.split("<url>")
            url = [a[i].split("</url>")[0] for i in range(1,len(a))]
            ext = "." + url[0].split(".")[-1]
            b = xmlStream.split("<title><![CDATA[")
            name = [b[i].split("]]></title>")[0] + ext for i in range(2,len(b))]
        else:
        # Si solo es una parte (compatiblidad con versiones ateriores):
            printt(u"[INFO] Vídeo único")
            url = "http://ondemand" + xmlStream.split("<url>http://ondemand")[1].split("<")[0]
            ext = "." + url.split(".")[-1]
